...
export class AuthResolver {
  ...
  async register(
    ...
    console.log('[AuthResolver] register', { email, hasPassword: !!password, hasName: !!name });
   ...
   */
  @Mutation(() => AuthPayload, { description: 'Login with email and password (password optional for backward compatibility)' })
  async login(
    @Args('email') email: string,
   ...
   */
  @Mutation(() => MessageResponse, { description: 'Change password (requires authentication)' })
   ...
   * Token must be valid and not expired.
   */
  @Mutation(() => MessageResponse, { description: 'Verify email address using verification token' })
